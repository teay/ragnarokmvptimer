...
/// Merge remote MVPs from Firebase with original server data.
/// This ensures we have full spawn info and correct respawn calculations.
    ...
    remote_mvps
        ...
        .map(|remote| {
            if let Some(original) = original_all.iter().find(|o| o.id == remote.id) {
                // Filter spawn to only the death map if available
