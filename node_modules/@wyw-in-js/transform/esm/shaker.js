...
export const shaker = (evalConfig, ast, code, {
  ...
  const plugins = [...preShakePlugins, [shakerPlugin, config], ...(evalConfig.plugins ?? []).filter(i => !hasKeyInList(i, highPriorityPlugins))];
  ...
  if (evalConfig.filename?.endsWith('.ts') || evalConfig.filename?.endsWith('.tsx') || evalConfig.filename?.endsWith('.mts') || evalConfig.filename?.endsWith('.cts')) {
